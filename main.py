...
def main():
    ...
    parser.add_argument("--mode", type=str, required=True, choices=["train", "predict", "download"],
                        help="Mode to run the script: 'train' to train the model, 'predict' to test inference, 'download' to fetch pre-trained model.")
    ...
    if args.mode == "train":
        ...
        try:
            model_dir = "models/roberta_goemotions_model" if os.path.exists("models/roberta_goemotions_model") else "models"
            tokenizer_dir = "models/roberta_goemotions_tokenizer" if os.path.exists("models/roberta_goemotions_tokenizer") else model_dir
            predictor = EmotionPredictor(model_path=model_dir, tokenizer_path=tokenizer_dir)
